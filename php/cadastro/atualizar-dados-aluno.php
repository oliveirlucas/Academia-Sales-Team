		...
		if($modalidade == "Selecione a modalidade"){
			...
			if($modalidade == "Muay Thai"){
                if($nivel == "Selecione a Prajied" || $nivel == "-- ADULTO --" || $nivel == "-- KIDS --"){
					...
					$result_usuario = "UPDATE aluno SET NOM_ALUNO='$nome_aluno' ,CPF='$cpf_aluno',RG='$rg_aluno',DATA='$data_aluno',NIVEL='$nivel',COD_MODALIDADE = 1,STATUS='$status' 
				...
				$result_usuario = "UPDATE aluno SET NOM_ALUNO='$nome_aluno' ,CPF='$cpf_aluno',RG='$rg_aluno',DATA='$data_aluno',NIVEL='$nivel',COD_MODALIDADE = 2,STATUS='$status' 
									WHERE COD_ALUNO = '$matricula_sim'";
				$resultado_usuario = mysqli_query($connect, $result_usuario);
		...
		if($modalidade_nao =="Selecione a modalidade"){
			...
			if($modalidade_nao == "Muay Thai"){
                if($nivel_nao == "Selecione a Prajied" || $nivel_nao == "-- ADULTO --" || $nivel_nao == "-- KIDS --"){
					...
					$result_usuario = "UPDATE aluno SET NOM_ALUNO='$nome_aluno_nao',TELEFONE='$telefone_nao',EMAIL='$email_nao',CPF='$cpf_nao',RG='$rg_nao',DATA='$data_nao',ENDERECO='$endereco_nao Nº".$numero_nao."',CIDADE='$cidade_nao',ESTADO='$estado_nao',NIVEL='$nivel_nao',COD_MODALIDADE = 2,STATUS='$status_nao',BAIRRO='$bairro_nao',CEP='$cep_nao' WHERE COD_ALUNO = '$matricula_nao'";
					$resultado_usuario = mysqli_query($connect, $result_usuario);
